image_name = leddzip/github-gh-cli
...
SHELL = /bin/bash
generate-dockerfile:
	echo "export INJECT_LEDDZIP_UBUNTU_GIT_DOCKER_TAG=$(ubuntu_tag)" > args
	source args
...
test:
	source args
	bash ./build-scripts/test-gh.sh $(image_name):$(env_prefix)$(ubuntu_tag)
